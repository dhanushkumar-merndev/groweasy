...
import { sanitizeCellValue } from "@/lib/formatting"
import type { ImportSheet, RawImportRow, RowData, ValidationResult, ValidationWarning } from "@/lib/types"
...
export function parseWorkbook(buffer: ArrayBuffer, options: ParseOptions): ValidationResult {
  ...
  workbook.SheetNames.forEach((sheetName, sheetIndex) => {
    ...
    const matrix = XLSX.utils.sheet_to_json<CellValueMatrix>(worksheet, {
      header: 1,
      blankrows: false,
      defval: "",
      raw: false,
...
type CellValueMatrix = Array<string | number | boolean | null>

function normalizeHeaders(headerRow: CellValueMatrix) {
...
function rowFromCells(headers: string[], cells: CellValueMatrix, dashValuesBlank: boolean): RowData {
  const row: RowData = {}
...
function countFormulaLikeCells(cells: CellValueMatrix) {
